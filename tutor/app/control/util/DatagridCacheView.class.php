...
class DatagridCacheView extends TPage
{
    ...
    public function __construct()
    {
        ...
        // add the columns
        $this->datagrid->addQuickColumn('Variable',  'key',      'left', 200);
        $this->datagrid->addQuickColumn('Hits',      'nhits',    'left', 100);
        $mt = $this->datagrid->addQuickColumn('Mod. Time', 'mtime',    'left', 180);
        $this->datagrid->addQuickColumn('Memory',    'mem_size', 'left', 160);
    ...
    public static function onView($param)
    {
        ...
        $div->add( str_replace("\n", '<br>', print_r(unserialize(apc_fetch( $param['key'] )), TRUE)) );
